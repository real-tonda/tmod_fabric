...
public class PingCommand {
    ...
    private static int executeOther(CommandContext<ServerCommandSource> context) {
        ...
        source.sendFeedback(() -> Text.literal(
            "§7" + targetName + "'s ping: " + pingColor + ping + "ms"
        ), false);
    ...
    private static String getPingColor(int ping) {
        if (ping <= 50) return "§a"; // Green - Excellent
        if (ping <= 100) return "§e"; // Yellow - Good
        if (ping <= 150) return "§6"; // Gold - Fair
        if (ping <= 250) return "§c"; // Red - Poor
