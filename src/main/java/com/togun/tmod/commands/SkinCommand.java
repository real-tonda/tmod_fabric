...
public class SkinCommand {
    ...
    private static int setSkinSelf(CommandContext<ServerCommandSource> context) {
        ...
        SkinManager.setSkin(player, targetPlayerName).thenAccept(success -> {
            if (success) {
                source.sendFeedback(
                        () -> Text.literal(
                                "§aSkin updated to " + targetPlayerName + "'s skin! §eReconnect to see changes."),
                        false);
            } else {
                source.sendError(
                        Text.literal("§cFailed to fetch skin for " + targetPlayerName + ". Player may not exist."));
            }
        });
    ...
    private static int setSkinOther(CommandContext<ServerCommandSource> context) {
        ...
        SkinManager.setSkin(targetPlayer, targetPlayerName).thenAccept(success -> {
            if (success) {
                source.sendFeedback(() -> Text.literal("§aUpdated " + targetPlayer.getName().getString() + "'s skin to "
                        + targetPlayerName + "'s skin!"), false);
                targetPlayer.sendMessage(Text.literal("§aYour skin has been updated to " + targetPlayerName
                        + "'s skin! §eReconnect to see changes."));
            } else {
                source.sendError(
                        Text.literal("§cFailed to fetch skin for " + targetPlayerName + ". Player may not exist."));
            }
    ...
    private static int clearSkinSelf(CommandContext<ServerCommandSource> context) {
        ...
        source.sendFeedback(() -> Text.literal("§aYour skin has been restored to default! §eReconnect to see changes."),
                false);
    ...
    private static int clearSkinOther(CommandContext<ServerCommandSource> context) {
        ...
        source.sendFeedback(() -> Text.literal("§aCleared " + targetPlayer.getName().getString() + "'s custom skin!"),
                false);
    ...
    private static int updateSkinOther(CommandContext<ServerCommandSource> context) {
        ...
        source.sendFeedback(() -> Text.literal("§aRefreshed " + targetPlayer.getName().getString() + "'s skin!"),
                false);
